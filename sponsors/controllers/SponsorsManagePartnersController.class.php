<?php
/*##################################################
 *                      SponsorsManagePartnersController.class.php
 *                            -------------------
...
class SponsorsManagePartnersController extends AdminModuleController
{
	private $lang;
	private $view;
	...
	private function build_table()
	{
		...
		foreach ($result as $row)
		{
			...
			$edit_link = new LinkHTMLElement(SponsorsUrlBuilder::edit($partner->get_id()), '', array('title' => LangLoader::get_message('edit', 'common')), 'fa fa-edit');
			$delete_link = new LinkHTMLElement(SponsorsUrlBuilder::delete($partner->get_id()), '', array('title' => LangLoader::get_message('delete', 'common'), 'data-confirmation' => 'delete-element'), 'fa fa-delete');

			$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
			$author = $user->get_id() !== User::VISITOR_LEVEL ? new LinkHTMLElement(UserUrlBuilder::profile($user->get_id()), $user->get_display_name(), (!empty($user_group_color) ? array('style' => 'color: ' . $user_group_color) : array()), UserService::get_level_class($user->get_level())) : $user->get_display_name();

			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(SponsorsUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $partner->get_id(), $partner->get_rewrited_name()), $partner->get_name()), 'left'),
				...
				new HTMLTableRowCell($partner->get_creation_date()->format(Date::FORMAT_DAY_MONTH_YEAR_HOUR_MINUTE)),
				new HTMLTableRowCell($partner->get_status()),
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->lang['sponsors.management'], $this->lang['module_title']);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(SponsorsUrlBuilder::manage());

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['module_title'], SponsorsUrlBuilder::home());

		$breadcrumb->add($this->lang['sponsors.management'], SponsorsUrlBuilder::manage());
