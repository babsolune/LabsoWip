<?php
/*##################################################
 *                          AgendaAjaxAgendaController.class.php
 *                            -------------------
...
class AgendaAjaxAgendaController extends AbstractController
{
	private $lang;
	private $view;
	private $mini_agenda = false;
	private $year;
	private $month;
	
	public function set_mini_agenda()
	{
		$this->mini_agenda = true;
	}
	
	public function is_mini_agenda()
	{
		return $this->mini_agenda;
	}
	...
	private function build_view(HTTPRequestCustom $request)
	{
		...
		$categories = AgendaService::get_categories_manager()->get_categories_cache()->get_categories();
		
		$year = $this->year ? $this->year : min($request->get_int('agenda_ajax_year', date('Y')), 2037);
		$month = $this->month ? $this->month : min($request->get_int('agenda_ajax_month', date('n')), 12);
		...
		$events = $month == date('n') && $year == date('Y') ? AgendaCurrentMonthEventsCache::load()->get_events() : AgendaService::get_all_current_month_events($month, $year, $month_days);
		...
		foreach ($events as $event)
		{
			...
			$end_date = !empty($event['end_date']) ? new Date($event['end_date'], Timezone::SERVER_TIMEZONE) : $start_date;
...
' : '') . ($event['type'] != 'BIRTHDAY' ? (($j == $start_date->get_day() && $month == $start_date->get_month() && $year == $start_date->get_year()) ? $start_date->get_hours() . 'h' . $start_date->get_minutes() . ' : ' : '') : LangLoader::get_message('agenda.labels.birthday_title', 'common', 'agenda') . ' ') . $event['title'],
						'type' => $event['type'],
					...
					if ($event['type'] == 'BIRTHDAY')
					{
						$events_legends_list[$j] = array(
							'name' => LangLoader::get_message('agenda.labels.birthday', 'common', 'agenda'),
					...
					{
						$events_legends_list[$j] = array(
							'name' => LangLoader::get_message('agenda.titles.event', 'common', 'agenda'),
		...
		$this->view->put_all(array(
			'C_MINI_MODULE' => $this->is_mini_agenda(),
			'C_DISPLAY_LEGEND' => !empty($events_legends_list) && !$this->is_mini_agenda(),
			'DATE' => $array_l_month[$month - 1] . ' ' . $year,
			'MINI_MODULE' => (int)$this->is_mini_agenda(),
			...
			'U_AJAX_AGENDA' => AgendaUrlBuilder::ajax_month_agenda()->rel(),
			'U_AJAX_EVENTS' => AgendaUrlBuilder::ajax_month_events()->rel()
		));
		...
		for ($i = 1; $i <= 56; $i++)
		{
			...
			$this->view->assign_block_vars('day', array(
				'C_MONTH_DAY' => ($i % 8) != 1 && $class != 'agenda-none',
				...
				'U_DAY_EVENTS' => AgendaUrlBuilder::home($year, $month, $today, true)->rel()
			));
		}
	}
	...
	{
		$legend_view = new FileTemplate('agenda/AgendaLegend.tpl');
	...
	private function init(HTTPRequestCustom $request)
	{
		...
		$this->view = new FileTemplate('agenda/AgendaAjaxAgendaController.tpl');
		$this->view->add_lang($this->lang);
		
		if ($request->has_getparameter('agenda_mini') && $request->get_getvalue('agenda_mini') == 1)
			$this->set_mini_agenda();
	}
