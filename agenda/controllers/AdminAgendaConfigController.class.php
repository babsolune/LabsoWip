<?php
/*##################################################
 *                              AdminAgendaConfigController.class.php
 *                            -------------------
...
class AdminAgendaConfigController extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		return new AdminAgendaDisplayResponse($tpl, $this->lang['module_config_title']);
	}
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldNumberEditor('items_number_per_page', $this->lang['agenda.config.items_number_per_page'], $this->config->get_items_number_per_page(),
		...
		$fieldset->add_field(new FormFieldColorPicker('event_color', $this->lang['agenda.config.event_color'], $this->config->get_event_color(),
			array(),
			array(new FormFieldConstraintRegex('`^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$`i'))
		));

		$fieldset->add_field(new FormFieldCheckbox('members_birthday_enabled', $this->lang['agenda.config.members_birthday_enabled'], $this->config->is_members_birthday_enabled(),
		...
		{
			$fieldset->add_field(new FormFieldHTML('user_born_disabled_msg', MessageHelper::display($this->lang['agenda.error.e_user_born_field_disabled'], MessageHelper::WARNING)->render()));
		}

		$fieldset->add_field(new FormFieldColorPicker('birthday_color', $this->lang['agenda.config.birthday_color'], $this->config->get_birthday_color(),
		...
		//Documentation
        $doc_fieldset = new FormFieldsetHTML('documentation', $this->lang['agenda.documentation']);
	...
	private function save()
	{
		...
		// 		NotationService::update_notation_scale('agenda', $this->config->get_notation_scale(), $this->form->get_value('notation_scale'));
		// }
		// else
		...
		AgendaConfig::save();
		AgendaService::get_categories_manager()->regenerate_cache();
