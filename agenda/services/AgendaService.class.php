 ...
 * @desc Services of the agenda module
 */
class AgendaService
{
	 ...
	 * @param string[] $event new AgendaEvent
	 */
	public static function add_event(AgendaEvent $event)
	{
		$result = self::$db_querier->insert(AgendaSetup::$agenda_events_table, $event->get_properties());
	 ...
	 * @param string[] $event_content new AgendaEventContent
	 */
	public static function add_event_content(AgendaEventContent $event_content)
	{
		$result = self::$db_querier->insert(AgendaSetup::$agenda_events_content_table, $event_content->get_properties());
	...
	{
		self::$db_querier->insert(AgendaSetup::$agenda_users_relation_table, array(
			'event_id' => $event_id,
			'user_id' => $user_id
		));
	}
	 ...
	 * @param string[] $event AgendaEvent to update
	 */
	public static function update_event(AgendaEvent $event)
	{
		self::$db_querier->update(AgendaSetup::$agenda_events_table, $event->get_properties(), 'WHERE id_event = :id', array(
			'id' => $event->get_id()
		));
	 ...
	 * @param string[] $event_content AgendaEventContent to update
	 */
	public static function update_event_content(AgendaEventContent $event_content)
	{
		self::$db_querier->update(AgendaSetup::$agenda_events_content_table, $event_content->get_properties(), 'WHERE id = :id', array(
	...
	{
		self::$db_querier->delete(AgendaSetup::$agenda_events_table, $condition, $parameters);
	}
	...
	{
		self::$db_querier->delete(AgendaSetup::$agenda_events_content_table, $condition, $parameters);
	}
	...
	{
		self::$db_querier->delete(AgendaSetup::$agenda_users_relation_table, 'WHERE event_id = :id', array(
			'id' => $event_id
		));
	}
	...
	{
		self::$db_querier->delete(AgendaSetup::$agenda_users_relation_table, 'WHERE event_id = :event_id AND user_id = :user_id', array(
			'event_id' => $event_id,
			'user_id' => $user_id
		));
	}
	...
	{
		$authorized_categories = AgendaService::get_authorized_categories($id_category);
		
		return self::$db_querier->select((AgendaConfig::load()->is_members_birthday_enabled() ? "
		(SELECT member_extended_fields.user_born AS start_date, member_extended_fields.user_born AS end_date, display_name AS title, 'BIRTHDAY' AS type, 0 AS id_category, '" . AgendaEventContent::YEARLY . "' AS repeat_type, 100 AS repeat_number
	...
	public static function get_categories_manager()
	{
		if (self::$categories_manager === null)
		{
			...
			$categories_items_parameters->set_table_name_contains_items(AgendaSetup::$agenda_events_content_table);
			self::$categories_manager = new CategoriesManager(AgendaCategoriesCache::load(), $categories_items_parameters);
		}
