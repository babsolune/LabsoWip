...
class ModmixSearchable extends AbstractSearchableExtensionPoint
{
	public function get_search_request($args)
	{
		...
		return "SELECT " . $args['id_search'] . " AS id_search,
			...
			CONCAT('" . PATH_TO_ROOT . "/modmix/" . (!ServerEnvironmentConfig::load()->is_url_rewriting_enabled() ? "index.php?url=/" : "") . "', id_category, '-', IF(id_category != 0, cat.rewrited_name, 'root'), '/', modmix.id, '-', modmix.rewrited_title) AS link
