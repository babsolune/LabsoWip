<?php
/*##################################################
 *                   AdminPortfolioConfigController.class.php
 *                            -------------------
...
class AdminPortfolioConfigController extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		return new AdminPortfolioDisplayResponse($tpl, $this->lang['module.config.title']);
	}
	...
	private function build_form()
	{
		...
		$fieldset = new FormFieldsetHTML('portfolio.configuration', LangLoader::get_message('configuration', 'admin-common'));
		...
		$fieldset->add_field(new FormFieldCheckbox('display_icon_cats', $this->lang['portfolio.configuration.cats.icon.display'], $this->config->are_cat_icons_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('display_sort_filters', $this->lang['portfolio.configuration.sort.filter.display'], $this->config->are_sort_filters_enabled()
		));

		$fieldset->add_field(new FormFieldCheckbox('enabled_items_suggestions', $this->lang['portfolio.configuration.suggestions.display'], $this->config->get_enabled_items_suggestions(),
		...
		$fieldset->add_field(new FormFieldNumberEditor('suggested_items_nb', $this->lang['portfolio.configuration.suggestions.nb'], $this->config->get_suggested_items_nb(),
			array('min' => 1, 'max' => 10, 'hidden' => !$this->config->get_enabled_items_suggestions()),
			array(new FormFieldConstraintIntegerRange(1, 10))
		));

		$fieldset->add_field(new FormFieldCheckbox('enabled_navigation_links', $this->lang['portfolio.configuration.navigation.links.display'], $this->config->get_enabled_navigation_links(),
			array('description' => $this->lang['portfolio.configuration.navigation.links.display.desc'])
		));

		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['portfolio.configuration.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['portfolio.configuration.mosaic.type.display'], PortfolioConfig::MOSAIC_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['portfolio.configuration.list.type.display'], PortfolioConfig::LIST_DISPLAY),
				new FormFieldSelectChoiceOption($this->lang['portfolio.configuration.table.type.display'], PortfolioConfig::TABLE_DISPLAY)
			)
		));

		$fieldset->add_field(new FormFieldNumberEditor('characters_number_to_cut', $this->lang['portfolio.configuration.characters.number.to.cut'], $this->config->get_characters_number_to_cut(),
			array('min' => 20, 'max' => 1000, 'required' => true),
			array(new FormFieldConstraintIntegerRange(20, 1000))
		));

		$fieldset->add_field(new FormFieldCheckbox('display_descriptions_to_guests', $this->lang['portfolio.configuration.display.descriptions.to.guests'], $this->config->are_descriptions_displayed_to_guests()));
	...
	private function save()
	{
		...
		PortfolioService::get_categories_manager()->regenerate_cache();
	}
}
?>
