<?php
/*##################################################
 *		       PortfolioDisplayTagController.class.php
 *                            -------------------
...
class PortfolioDisplayTagController extends ModuleController
{
	private $lang;
	private $view;
	private $keyword;
	...
	private function get_keyword()
	{
		if ($this->keyword === null)
		{
			...
			{
				try {
					$this->keyword = PortfolioService::get_keywords_manager()->get_keyword('WHERE rewrited_name=:rewrited_name', array('rewrited_name' => $rewrited_name));
	...
	private function build_view($request)
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT portfolio.*, member.*, com.number_comments, notes.number_notes, notes.average_notes, note.note
		FROM ' . PortfolioSetup::$portfolio_table . ' portfolio
		LEFT JOIN ' . DB_TABLE_KEYWORDS_RELATIONS . ' relation ON relation.module_id = \'portfolio\' AND relation.id_in_module = portfolio.id
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. PortfolioUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination = new ModulePagination($page, $result['nbr_items'], PortfolioConfig::load()->get_items_number_per_page());
		$pagination->set_url(PortfolioUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), $field, $mode, '%d'));
	...
	{
		if (!(PortfolioAuthorizationsService::check_authorizations()->read()))
		{
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->get_keyword()->get_name(), $this->lang['portfolio.module.title']);
		$graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['portfolio.seo.description.tag'], array('subject' => $this->get_keyword()->get_name())));
		$graphical_environment->get_seo_meta_data()->set_canonical_url(PortfolioUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['portfolio.module.title'], PortfolioUrlBuilder::home());
		$breadcrumb->add($this->get_keyword()->get_name(), PortfolioUrlBuilder::display_tag($this->get_keyword()->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'),AppContext::get_request()->get_getint('page', 1)));
