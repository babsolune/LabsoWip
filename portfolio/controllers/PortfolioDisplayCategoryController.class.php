<?php
/*##################################################
 *                      PortfolioDisplayCategoryController.class.php
 *                            -------------------
...
class PortfolioDisplayCategoryController extends ModuleController
{
	private $lang;
	private $config;
	...
	private function init()
	{
		...
		$this->view = new FileTemplate('portfolio/PortfolioDisplayCategoryController.tpl');
		$this->view->add_lang($this->lang);
		$this->config = PortfolioConfig::load();
		$this->comments_config = new PortfolioComments();
		$this->notation_config = new PortfolioNotation();
	}
	...
	private function build_view()
	{
		...
		$mode = $request->get_getstring('sort', PortfolioUrlBuilder::DEFAULT_SORT_MODE);
		$field = $request->get_getstring('field', PortfolioUrlBuilder::DEFAULT_SORT_FIELD);
	...
	private function build_items_listing_view(Date $now, $field, $mode, $page, $subcategories_page)
	{
		...
		$result = PersistenceContext::get_querier()->select('SELECT portfolio.*, member.*, com.number_comments, notes.average_notes, notes.number_notes, note.note
		FROM ' . PortfolioSetup::$portfolio_table . ' portfolio
		LEFT JOIN ' . DB_TABLE_MEMBER . ' member ON member.user_id = portfolio.author_user_id
		LEFT JOIN ' . DB_TABLE_COMMENTS_TOPIC . ' com ON com.id_in_module = portfolio.id AND com.module_id = \'portfolio\'
		LEFT JOIN ' . DB_TABLE_AVERAGE_NOTES . ' notes ON notes.id_in_module = portfolio.id AND notes.module_name = \'portfolio\'
		LEFT JOIN ' . DB_TABLE_NOTE . ' note ON note.id_in_module = portfolio.id AND note.module_name = \'portfolio\' AND note.user_id = :user_id
		' . $condition . '
		...
		$this->view->put_all(array(
			'C_MOSAIC' => $this->config->get_display_type() == PortfolioConfig::MOSAIC_DISPLAY,
			'C_LIST' => $this->config->get_display_type() == PortfolioConfig::LIST_DISPLAY,
			'C_TABLE' => $this->config->get_display_type() == PortfolioConfig::TABLE_DISPLAY,
			...
			'C_MODERATION' => PortfolioAuthorizationsService::check_authorizations($this->get_category()->get_id())->moderation(),
			...
			'CATEGORY_ID' => $this->get_category()->get_id(),
			'U_EDIT_CATEGORY' => $this->get_category()->get_id() == Category::ROOT_CATEGORY ? PortfolioUrlBuilder::configuration()->rel() : PortfolioUrlBuilder::edit_category($this->get_category()->get_id())->rel()
		));
		...
		while($row = $result->fetch())
		{
			...
			$this->build_keywords_view($work);

			$this->view->assign_block_vars('items', $work->get_array_tpl_vars());
			$this->build_sources_view($work);
		}
		$result->dispose();
	}

	private function build_sources_view(Work $work)
	{
		$sources = $work->get_sources();
	...
	{
		$subcategories = PortfolioService::get_categories_manager()->get_categories_cache()->get_children($this->get_category()->get_id(), PortfolioService::get_authorized_categories($this->get_category()->get_id()));
		...
		foreach ($subcategories as $id => $category)
		{
			...
			if ($cats_displayed_nbr > $subcategories_pagination->get_display_from() && $cats_displayed_nbr <= ($subcategories_pagination->get_display_from() + $subcategories_pagination->get_number_items_per_page()))
			{
				...
				$this->view->assign_block_vars('sub_categories', array(
					...
					'U_CATEGORY' => PortfolioUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name())->rel()
				));
			}
		}
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			array('events' => array('change' => 'document.location = "'. PortfolioUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_category()
	{
		if ($this->category === null)
		{
			...
			{
				try {
					$this->category = PortfolioService::get_categories_manager()->get_categories_cache()->get_category($id);
			...
			{
				$this->category = PortfolioService::get_categories_manager()->get_categories_cache()->get_category(Category::ROOT_CATEGORY);
			}
		}
		return $this->category;
	}

	private function build_keywords_view(Work $work)
	{
		$keywords = $work->get_keywords();
	...
	{
		$number_items = PersistenceContext::get_querier()->count(PortfolioSetup::$portfolio_table, $condition, $parameters);

		$pagination = new ModulePagination($page, $number_items, (int)PortfolioConfig::load()->get_items_number_per_page());
		$pagination->set_url(PortfolioUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, '%d', $subcategories_page));
	...
	private function get_subcategories_pagination($subcategories_number, $categories_number_per_page, $field, $mode, $page, $subcategories_page)
	{
		...
		$pagination->set_url(PortfolioUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), $field, $mode, $page, '%d'));
	...
	private function check_authorizations()
	{
		...
		{
			if (!PortfolioAuthorizationsService::check_authorizations($this->get_category()->get_id())->read())
			{
				$error_controller = PHPBoostErrors::user_not_authorized();
				DispatchManager::redirect($error_controller);
			}
	...
	private function generate_response()
	{
		...
		if ($this->category->get_id() != Category::ROOT_CATEGORY)
			$graphical_environment->set_page_title($this->category->get_name(), $this->lang['portfolio.module.title']);
		else
			$graphical_environment->set_page_title($this->lang['portfolio.module.title']);

		$graphical_environment->get_seo_meta_data()->set_description($this->category->get_description());
		$graphical_environment->get_seo_meta_data()->set_canonical_url(PortfolioUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['portfolio.module.title'], PortfolioUrlBuilder::home());

		$categories = array_reverse(PortfolioService::get_categories_manager()->get_parents($this->category->get_id(), true));
		foreach ($categories as $id => $category)
		{
			if ($category->get_id() != Category::ROOT_CATEGORY)
				$breadcrumb->add($category->get_name(), PortfolioUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name(), AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));
		}
