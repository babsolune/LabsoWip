...
class Work
{
	...
	const DEFAULT_PICTURE = '/portfolio/templates/images/default.png';
	...
	{
		return PortfolioService::get_categories_manager()->get_categories_cache()->get_category($this->category_id);
	}
	...
	public function get_real_description()
	{
		...
		else
		{
			...
			return TextHelper::cut_string(@strip_tags($clean_contents[0], '<br><br/>'), (int)PortfolioConfig::load()->get_characters_number_to_cut());
		}
	}
	...
	{
		$now = new Date();
		return PortfolioAuthorizationsService::check_authorizations($this->category_id)->read() && ($this->get_publication_state() == self::PUBLISHED_NOW || ($this->get_publication_state() == self::PUBLICATION_DATE && $this->get_publication_start_date()->is_anterior_to($now) && ($this->enabled_end_date ? $this->get_publication_end_date()->is_posterior_to($now) : true)));
	}
	...
	public function get_keywords()
	{
		...
		{
			$this->keywords = PortfolioService::get_keywords_manager()->get_keywords($this->id);
		}
	...
	{
		return PortfolioAuthorizationsService::check_authorizations($this->category_id)->write() || PortfolioAuthorizationsService::check_authorizations($this->category_id)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return PortfolioAuthorizationsService::check_authorizations($this->category_id)->moderation() || ((PortfolioAuthorizationsService::check_authorizations($this->get_category_id())->write() || (PortfolioAuthorizationsService::check_authorizations($this->get_category_id())->contribution() && !$this->is_published())) && $this->get_author_user()->get_id() == AppContext::get_current_user()->get_id() && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL));
	}
	...
	public function set_properties(array $properties)
	{
		...
		$notation->set_module_name('portfolio');
	...
	public function get_array_tpl_vars()
	{
		...
		return array_merge(
			...
			'C_READ_MORE'                      => !$this->get_description_enabled() && TextHelper::strlen($contents) > PortfolioConfig::load()->get_characters_number_to_cut() && $description != @strip_tags($contents, '<br><br/>'),
			...
			'L_COMMENTS'         => CommentsService::get_number_and_lang_comments('portfolio', $this->get_id()),
			'COMMENTS_NUMBER'    => CommentsService::get_number_comments('portfolio', $this->get_id()),
			'VIEWS_NUMBER'       => $this->get_views_number(),
			'VISITS_NUMBER'      => $this->get_visits_number(),
			'FILE_SIZE' 		 => $file_size,
			'DOWNLOADS_NUMBER'   => $this->get_downloads_number(),
			...
			'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? PortfolioUrlBuilder::configuration()->rel() : PortfolioUrlBuilder::edit_category($category->get_id())->rel(),

			//Links
			'U_COMMENTS'    => PortfolioUrlBuilder::display_items_comments($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			...
			'U_ITEM'        => PortfolioUrlBuilder::display_item($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			'U_WEBLINK' 	=> PortfolioUrlBuilder::visit_item($this->id)->rel(),
			'U_FILE' 		=> PortfolioUrlBuilder::download_item($this->id)->rel(),
			'U_EDIT_ITEM'   => PortfolioUrlBuilder::edit_item($this->id, AppContext::get_request()->get_getint('page', 1))->rel(),
			'U_DELETE_ITEM' => PortfolioUrlBuilder::delete_item($this->id)->rel(),
			'U_DEADLINK' 	=> PortfolioUrlBuilder::dead_link($this->id)->rel(),
			'U_SYNDICATION' => PortfolioUrlBuilder::category_syndication($category->get_id())->rel(),
			'U_PRINT_ITEM'  => PortfolioUrlBuilder::print_item($this->get_id(), $this->get_rewrited_title())->rel()
			)
		);
	}
}
