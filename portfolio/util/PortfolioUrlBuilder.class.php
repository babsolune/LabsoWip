 ...
 */
class PortfolioUrlBuilder
{
	const DEFAULT_SORT_FIELD = 'date';
	const DEFAULT_SORT_MODE = 'desc';

	private static $dispatcher = '/portfolio';
	...
	{
		return SyndicationUrlBuilder::rss('portfolio', $id);
	}
	 ...
	 * @return Url
	 */
	public static function print_item($id_work, $rewrited_title)
	{
		return DispatchManager::get_url(self::$dispatcher, '/print/' . $id_work . '-' .$rewrited_title . '/');
	}
	 ...
	 * @return Url
	 */
	public static function display_item($category_id, $rewrited_name_category, $id_work, $rewrited_title, $page = 1)
	{
		$page = $page !== 1 ? $page . '/' : '';
		return DispatchManager::get_url(self::$dispatcher, '/' . $category_id . '-' . $rewrited_name_category . '/' . $id_work . '-' .$rewrited_title . '/' . $page);
	}
	 ...
	 * @return Url
	 */
	public static function display_items_comments($category_id, $rewrited_name_category, $id_work, $rewrited_title)
	{
		return DispatchManager::get_url(self::$dispatcher, '/' . $category_id . '-' . $rewrited_name_category . '/' . $id_work . '-' . $rewrited_title . '/#comments-list');
	}
