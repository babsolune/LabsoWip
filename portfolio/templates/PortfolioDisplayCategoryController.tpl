<section id="module-portfolio">
	<header>
		<h1>
			<a href="${relative_url(SyndicationUrlBuilder::rss('portfolio', CATEGORY_ID))}" title="${LangLoader::get_message('syndication', 'common')}"><i class="fa fa-syndication"></i></a>
			# IF C_PENDING #{@portfolio.pending.items}# ELSE #{@portfolio.module.title}# IF NOT C_ROOT_CATEGORY # - {CATEGORY_NAME}# ENDIF ## ENDIF # # IF C_CATEGORY ## IF IS_ADMIN #<a href="{U_EDIT_CATEGORY}" title="${LangLoader::get_message('edit', 'common')}"><i class="fa fa-edit smaller"></i></a># ENDIF ## ENDIF #
		</h1>
	...
	<div class="subcat-container elements-container# IF C_SEVERAL_CATS_COLUMNS # columns-{CATS_COLUMNS_NUMBER}# ENDIF #">
		...
		<div class="subcat-element block">
			<div class="subcat-content">
				...
				<br />
				<span class="small">{sub_categories.ITEMS_NUMBER} # IF sub_categories.C_MORE_THAN_ONE_ITEM #${TextHelper::lcfirst(LangLoader::get_message('portfolio.items', 'common', 'portfolio'))}# ELSE #${TextHelper::lcfirst(LangLoader::get_message('portfolio.item', 'common', 'portfolio'))}# ENDIF #</span>
			</div>
		</div>
		# END sub_categories #
	</div>
	...
	# ELSE #
		...
		# ELSE #
			...
			<div class="elements-container# IF C_SEVERAL_COLUMNS # columns-{COLUMNS_NUMBER}# ENDIF#">
				# START items #
					<article id="portfolio-items-{items.ID}" class="portfolio-items several-items# IF C_MOSAIC # block# ENDIF ## IF items.C_NEW_CONTENT # new-content# ENDIF #" itemscope="itemscope" itemtype="http://schema.org/CreativeWork">
						<header>
							<h2>
