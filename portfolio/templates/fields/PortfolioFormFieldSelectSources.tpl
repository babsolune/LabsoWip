<script>
<!--
var PortfolioFormFieldSelectSources = function(){
	this.integer = {NBR_FIELDS};
	this.id_input = ${escapejs(ID)};
	this.max_input = {MAX_INPUT};
};

PortfolioFormFieldSelectSources.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:PortfolioFormFieldSelectSources.delete_field('+ this.integer +');'}).html('<i class="fa fa-delete"></i>').appendTo('#' + id);
...
var PortfolioFormFieldSelectSources = new PortfolioFormFieldSelectSources();
-->
</script>
...
# START fieldelements #
		<div id="${escape(ID)}_{fieldelements.ID}">
			...
			<a href="javascript:PortfolioFormFieldSelectSources.delete_field({fieldelements.ID});" data-confirmation="delete-element"><i class="fa fa-delete"></i></a>
		</div>
# END fieldelements #
</div>
<a href="javascript:PortfolioFormFieldSelectSources.add_field();" id="add-${escape(ID)}" class="field-source-more-value" title="${LangLoader::get_message('add', 'common')}"><i class="fa fa-plus"></i></a>
