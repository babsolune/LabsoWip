<section id="module-portfolio">
	<header>
		<h1>
			...
			{@portfolio.module.title}# IF NOT C_ROOT_CATEGORY # - {CATEGORY_NAME}# ENDIF # # IF IS_ADMIN #<a href="{U_EDIT_CATEGORY}" title="${LangLoader::get_message('edit', 'common')}"><i class="fa fa-edit smaller"></i></a># ENDIF #
		</h1>
	</header>
	# INCLUDE NOT_VISIBLE_MESSAGE #
	<article itemscope="itemscope" itemtype="http://schema.org/Work" id="article-portfolio-{ID}" class="article-portfolio# IF C_NEW_CONTENT # new-content# ENDIF #">
		<header>
			<h2>
			...
			<div class="more">
				...
				&nbsp;<i class="fa fa-eye" title="{VIEWS_NUMBER} {@portfolio.sort.field.views}"></i>&nbsp;<span title="{VIEWS_NUMBER} {@portfolio.sort.field.views}">{VIEWS_NUMBER}</span>
		...
		<div class="content">
			...
			# IF C_WEBLINK #
				<hr />
				<span class="actions">
					<i class="fa fa-sign-out" title="{@portfolio.visits.number}"></i> {VISITS_NUMBER}
				</span>
				<a href="{U_WEBLINK}" class="button submit">{@portfolio.visit}</a>
				<div class="spacer"></div>
			# ENDIF #
			...
			# IF C_VISIBLE_LINKS #
				# IF C_FILE #
					<hr />
					<span class="actions">
						<i class="fa fa-microchip" title="{@portfolio.visits.number}"></i> {FILE_SIZE} |
						<i class="fa fa-download" title="{@portfolio.downloads.number}"></i> {DOWNLOADS_NUMBER}
					</span>
					<a href="{U_FILE}" class="button submit">{@portfolio.file}</a>
					<div class="spacer"></div>
				# ENDIF #
			# ENDIF #
			...
			# IF C_USER_CONNECTED #
				# IF C_DEADLINK #
					...
					<a href="{U_DEADLINK}" class="button alt">{@portfolio.dead.link}</a>
					<div class="spacer"></div>
				# ENDIF #
			# ENDIF #
		</div>
				...
				<div class="elements-container columns-{SUGGESTED_COLUMNS} no-style">
					...
					<div class="block">
						<a href="{suggested_items.U_ITEM}">
							<figure class="suggested-thumbnail">
