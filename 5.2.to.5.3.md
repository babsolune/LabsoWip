...
- mise à jour du fichier table.css  
    - modification des header de table en bbcode  
Remplacer
```
...
`[container classe="responsive-table"][table]....[/table][/container]`
```
.responsive-table {
	max-width: 100%;
	overflow: auto;
...
`[container classe="bordered"][table]....[/table][/container]`
```
.bordered td {
...
# Tooltip
Afin de supprimer les title="" qui limitent les performences en mobile, un système de tooltip est mis en place sur les aria-label, ce qui permet l'affichage des texte d'icones (entre autres) en profitant des balise d'accessibilité
