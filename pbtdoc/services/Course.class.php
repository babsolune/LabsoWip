...
class Course
{
	...
	const DEFAULT_THUMBNAIL = '/pbtdoc/templates/images/default.png';
	...
	{
		return PbtdocService::get_categories_manager()->get_categories_cache()->get_category($this->id_category);
	}
	...
	public function get_real_description()
	{
		...
		else
		{
			...
			return TextHelper::cut_string(@strip_tags($clean_contents[0], '<br><br/>'), (int)PbtdocConfig::load()->get_number_character_to_cut());
		}
	}
	...
	{
		$now = new Date();
		return PbtdocAuthorizationsService::check_authorizations($this->id_category)->read() && ($this->get_publishing_state() == self::PUBLISHED_NOW || ($this->get_publishing_state() == self::PUBLISHED_DATE && $this->get_publishing_start_date()->is_anterior_to($now) && ($this->end_date_enabled ? $this->get_publishing_end_date()->is_posterior_to($now) : true)));
	}
	...
	public function get_keywords()
	{
		...
		{
			$this->keywords = PbtdocService::get_keywords_manager()->get_keywords($this->id);
		}
	...
	{
		return PbtdocAuthorizationsService::check_authorizations($this->id_category)->write() || PbtdocAuthorizationsService::check_authorizations($this->id_category)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return PbtdocAuthorizationsService::check_authorizations($this->id_category)->moderation() || ((PbtdocAuthorizationsService::check_authorizations($this->get_id_category())->write() || (PbtdocAuthorizationsService::check_authorizations($this->get_id_category())->contribution())));
	}
	...
	public function get_array_tpl_vars()
	{
		...
		return array_merge(
			...
			'C_READ_MORE'                     => !$this->get_description_enabled() && TextHelper::strlen($contents) > PbtdocConfig::load()->get_number_character_to_cut() && $description != @strip_tags($contents, '<br><br/>'),
			'C_SOURCES'                       => $nbr_sources > 0,
			'C_DIFFERED'                      => $this->published == self::PUBLISHED_DATE,
			'C_NEW_CONTENT'                   => ContentManagementConfig::load()->module_new_content_is_enabled_and_check_date('pbtdoc', $this->publishing_start_date != null ? $this->publishing_start_date->get_timestamp() : $this->get_date_created()->get_timestamp()) && $this->is_published(),

			//Pbtdoc
			'ID'                            => $this->get_id(),
			'TITLE'                         => $this->get_title(),
			'STATUS'                        => $this->get_status(),
			'L_COMMENTS'                    => CommentsService::get_number_and_lang_comments('pbtdoc', $this->get_id()),
			'NUMBER_COMMENTS'               => CommentsService::get_number_comments('pbtdoc', $this->get_id()),
			'NUMBER_VIEW'                   => $this->get_number_view(),
			...
			'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? PbtdocUrlBuilder::configuration()->rel() : PbtdocUrlBuilder::edit_category($category->get_id())->rel(),
			...
			'U_COURSE'        => PbtdocUrlBuilder::display_item($category->get_id(), $category->get_rewrited_name(), $this->get_id(), $this->get_rewrited_title())->rel(),
			'U_EDIT_COURSE'   => PbtdocUrlBuilder::edit_item($this->id, AppContext::get_request()->get_getint('page', 1))->rel(),
			'U_DELETE_COURSE' => PbtdocUrlBuilder::delete_item($this->id)->rel(),
			'U_SYNDICATION'    => PbtdocUrlBuilder::category_syndication($category->get_id())->rel(),
			'U_PRINT_COURSE'  => PbtdocUrlBuilder::print_item($this->get_id(), $this->get_rewrited_title())->rel()
			)
		);
	}
}
