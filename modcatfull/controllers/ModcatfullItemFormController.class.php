...
class ModcatfullItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		if (ModcatfullAuthorizationsService::check_authorizations($this->get_itemcatfull()->get_id_category())->moderation())
		{
		...
		if (ModcatfullService::get_categories_manager()->get_categories_cache()->has_categories())
		{
			...
			$fieldset->add_field(ModcatfullService::get_categories_manager()->get_select_categories_form_field('id_category', $this->common_lang['form.category'], $this->get_itemcatfull()->get_id_category(), $search_category_children_options));
		}
	...
	private function get_itemcatfull()
	{
		if ($this->itemcatfull === null)
		{
			...
			else
			{
				...
				$this->itemcatfull->init_default_properties(AppContext::get_request()->get_getint('id_category', Category::ROOT_CATEGORY));
			}
		}
	...
	private function save()
	{
		...
		if (ModcatfullService::get_categories_manager()->get_categories_cache()->has_categories())
			$itemcatfull->set_id_category($this->form->get_value('id_category')->get_raw_value());
		...
		if (!ModcatfullAuthorizationsService::check_authorizations($itemcatfull->get_id_category())->moderation())
		{
			if ($itemcatfull->get_id() === null)
			...
			if (ModcatfullAuthorizationsService::check_authorizations($itemcatfull->get_id_category())->contribution() && !ModcatfullAuthorizationsService::check_authorizations($itemcatfull->get_id_category())->write())
			...
			if ($this->is_contributor_member())
			{
				...
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						ModcatfullService::get_categories_manager()->get_heritated_authorizations($itemcatfull->get_id_category(), Category::MODERATION_AUTHORIZATIONS, Authorizations::AUTH_CHILD_PRIORITY),
	...
	private function build_response(View $tpl)
	{
		...
		if ($itemcatfull->get_id() === null)
		{
			$breadcrumb->add($this->lang['modcatfull.add'], ModcatfullUrlBuilder::add_item($itemcatfull->get_id_category()));
			...
			$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatfullUrlBuilder::add_item($itemcatfull->get_id_category()));
		}
		else
		{
			$categories = array_reverse(ModcatfullService::get_categories_manager()->get_parents($itemcatfull->get_id_category(), true));
