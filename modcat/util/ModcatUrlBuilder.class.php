...
class ModcatUrlBuilder
{
	 ...
	 * @return Url
	 */
	public static function display_item($id_category, $rewrited_name_category, $id_itemcat, $rewrited_title, $page = 1)
	{
		$page = $page !== 1 ? $page . '/' : '';
		return DispatchManager::get_url(self::$dispatcher, '/' . $id_category . '-' . $rewrited_name_category . '/' . $id_itemcat . '-' .$rewrited_title . '/' . $page);
	}

	/**
	 * @return Url
	 */
	public static function display_items_comments($id_category, $rewrited_name_category, $id_itemcat, $rewrited_title)
	{
		return DispatchManager::get_url(self::$dispatcher, '/' . $id_category . '-' . $rewrited_name_category . '/' . $id_itemcat . '-' . $rewrited_title . '/#comments-list');
	}
