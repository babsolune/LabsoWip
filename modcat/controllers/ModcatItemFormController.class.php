...
class ModcatItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		if (ModcatAuthorizationsService::check_authorizations($this->get_itemcat()->get_id_category())->moderation())
		{
		...
		if (ModcatService::get_categories_manager()->get_categories_cache()->has_categories())
		{
			...
			$fieldset->add_field(ModcatService::get_categories_manager()->get_select_categories_form_field('id_category', $this->common_lang['form.category'], $this->get_itemcat()->get_id_category(), $search_category_children_options));
		}
	...
	private function get_itemcat()
	{
		if ($this->itemcat === null)
		{
			...
			else
			{
				...
				$this->itemcat->init_default_properties(AppContext::get_request()->get_getint('id_category', Category::ROOT_CATEGORY));
			}
		}
		return $this->itemcat;
	}
	...
	private function save()
	{
		...
		if (ModcatService::get_categories_manager()->get_categories_cache()->has_categories())
			$itemcat->set_id_category($this->form->get_value('id_category')->get_raw_value());
		...
		if (!ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->moderation())
		{
			if ($itemcat->get_id() === null)
			...
			if (ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->contribution() && !ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->write())
			...
			if ($this->is_contributor_member())
			{
				...
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						ModcatService::get_categories_manager()->get_heritated_authorizations($itemcat->get_id_category(), Category::MODERATION_AUTHORIZATIONS, Authorizations::AUTH_CHILD_PRIORITY),
	...
	private function generate_response(View $tpl)
	{
		...
		if ($itemcat->get_id() === null)
		{
			$breadcrumb->add($this->lang['modcat.add'], ModcatUrlBuilder::add_item($itemcat->get_id_category()));
			...
			$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatUrlBuilder::add_item($itemcat->get_id_category()));
		}
		else
		{
			$categories = array_reverse(ModcatService::get_categories_manager()->get_parents($itemcat->get_id_category(), true));
