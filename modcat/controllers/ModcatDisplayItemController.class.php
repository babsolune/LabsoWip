...
class ModcatDisplayItemController extends ModuleController
{
	...
	private function build_suggested_items(Itemcat $itemcat)
	{
		...
		while ($row = $result->fetch())
		{
			...
			$this->tpl->assign_block_vars('suggested_items', array(
				...
				'U_ITEM' => ModcatUrlBuilder::display_item($row['id_category'], ModcatService::get_categories_manager()->get_categories_cache()->get_category($row['id_category'])->get_rewrited_name(), $row['id'], $row['rewrited_title'])->rel()
			));
		}
		$result->dispose();
	}
	...
	private function build_navigation_links(Itemcat $itemcat)
	{
		...
		FROM '. ModcatSetup::$modcat_table .'
		WHERE (published = 1 OR (published = 2 AND publication_start_date < :timestamp_now AND (publication_end_date > :timestamp_now OR publication_end_date = 0))) AND creation_date < :timestamp_itemcat AND id_category IN :authorized_categories ORDER BY creation_date DESC LIMIT 1 OFFSET 0)
		UNION
		(SELECT id, title, id_category, rewrited_title, thumbnail_url, \'NEXT\' as type
		FROM '. ModcatSetup::$modcat_table .'
		WHERE (published = 1 OR (published = 2 AND publication_start_date < :timestamp_now AND (publication_end_date > :timestamp_now OR publication_end_date = 0))) AND creation_date > :timestamp_itemcat AND id_category IN :authorized_categories ORDER BY creation_date ASC LIMIT 1 OFFSET 0)
		', array(
			'timestamp_now' => $now->get_timestamp(),
			'timestamp_itemcat' => $timestamp_itemcat,
			'authorized_categories' => array($itemcat->get_id_category())
		));
		...
		while ($row = $result->fetch())
		{
			...
			$this->tpl->put_all(array(
				...
				'U_'. $row['type'] .'_ITEM' => ModcatUrlBuilder::display_item($row['id_category'], ModcatService::get_categories_manager()->get_categories_cache()->get_category($row['id_category'])->get_rewrited_name(), $row['id'], $row['rewrited_title'])->rel(),
			));
		}
		$result->dispose();
	}
	...
	private function check_authorizations()
	{
		...
		$not_authorized = !ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->moderation() && !ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->write() && (!ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->contribution() || $itemcat->get_author_user()->get_id() != $current_user->get_id());

		switch ($itemcat->get_publication_state())
		{
			case Itemcat::PUBLISHED_NOW:
				if (!ModcatAuthorizationsService::check_authorizations($itemcat->get_id_category())->read())
				{
	...
	private function generate_response()
	{
		...
		$categories = array_reverse(ModcatService::get_categories_manager()->get_parents($this->itemcat->get_id_category(), true));
