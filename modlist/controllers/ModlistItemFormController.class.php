...
class ModlistItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		if (ModlistAuthorizationsService::check_authorizations($this->get_itemlist()->get_id_category())->moderation())
		{
		...
		if (ModlistService::get_categories_manager()->get_categories_cache()->has_categories())
		{
			...
			$fieldset->add_field(ModlistService::get_categories_manager()->get_select_categories_form_field('id_category', $this->common_lang['form.category'], $this->get_itemlist()->get_id_category(), $search_category_children_options));
		}
	...
	private function get_itemlist()
	{
		if ($this->itemlist === null)
		{
			...
			else
			{
				...
				$this->itemlist->init_default_properties(AppContext::get_request()->get_getint('id_category', Category::ROOT_CATEGORY));
			}
		}
	...
	private function save()
	{
		...
		if (ModlistService::get_categories_manager()->get_categories_cache()->has_categories())
			$itemlist->set_id_category($this->form->get_value('id_category')->get_raw_value());
		...
		if (!ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->moderation())
		{
			if ($itemlist->get_id() === null)
			...
			if (ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->contribution() && !ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->write())
			...
			if ($this->is_contributor_member())
			{
				...
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						ModlistService::get_categories_manager()->get_heritated_authorizations($itemlist->get_id_category(), Category::MODERATION_AUTHORIZATIONS, Authorizations::AUTH_CHILD_PRIORITY),
	...
	private function generate_response(View $tpl)
	{
		...
		if ($itemlist->get_id() === null)
		{
			$breadcrumb->add($this->lang['modlist.add'], ModlistUrlBuilder::add_item($itemlist->get_id_category()));
			...
			$graphical_environment->get_seo_meta_data()->set_canonical_url(ModlistUrlBuilder::add_item($itemlist->get_id_category()));
		}
		else
		{
			$categories = array_reverse(ModlistService::get_categories_manager()->get_parents($itemlist->get_id_category(), true));
