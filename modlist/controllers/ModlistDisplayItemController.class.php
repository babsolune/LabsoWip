...
class ModlistDisplayItemController extends ModuleController
{
	...
	private function build_suggested_items(Itemlist $itemlist)
	{
		...
		while ($row = $result->fetch())
		{
			...
			$this->tpl->assign_block_vars('suggested_items', array(
				...
				'U_ITEM' => ModlistUrlBuilder::display_item($row['id_category'], ModlistService::get_categories_manager()->get_categories_cache()->get_category($row['id_category'])->get_rewrited_name(), $row['id'], $row['rewrited_title'])->rel()
			));
		}
		$result->dispose();
	}
	...
	private function build_navigation_links(Itemlist $itemlist)
	{
		...
		FROM '. ModlistSetup::$modlist_table .'
		WHERE (published = 1 OR (published = 2 AND publication_start_date < :timestamp_now AND (publication_end_date > :timestamp_now OR publication_end_date = 0))) AND creation_date < :timestamp_itemlist AND id_category IN :authorized_categories ORDER BY creation_date DESC LIMIT 1 OFFSET 0)
		UNION
		(SELECT id, title, id_category, rewrited_title, thumbnail_url, \'NEXT\' as type
		FROM '. ModlistSetup::$modlist_table .'
		WHERE (published = 1 OR (published = 2 AND publication_start_date < :timestamp_now AND (publication_end_date > :timestamp_now OR publication_end_date = 0))) AND creation_date > :timestamp_itemlist AND id_category IN :authorized_categories ORDER BY creation_date ASC LIMIT 1 OFFSET 0)
		', array(
			'timestamp_now' => $now->get_timestamp(),
			'timestamp_itemlist' => $timestamp_itemlist,
			'authorized_categories' => array($itemlist->get_id_category())
		));
		...
		while ($row = $result->fetch())
		{
			...
			$this->tpl->put_all(array(
				...
				'U_'. $row['type'] .'_ITEM' => ModlistUrlBuilder::display_item($row['id_category'], ModlistService::get_categories_manager()->get_categories_cache()->get_category($row['id_category'])->get_rewrited_name(), $row['id'], $row['rewrited_title'])->rel(),
			));
		}
		$result->dispose();
	}
	...
	private function check_authorizations()
	{
		...
		$not_authorized = !ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->moderation() && !ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->write() && (!ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->contribution() || $itemlist->get_author_user()->get_id() != $current_user->get_id());

		switch ($itemlist->get_publication_state())
		{
			case Itemlist::PUBLISHED_NOW:
				if (!ModlistAuthorizationsService::check_authorizations($itemlist->get_id_category())->read())
				{
	...
	private function generate_response()
	{
		...
		$categories = array_reverse(ModlistService::get_categories_manager()->get_parents($this->itemlist->get_id_category(), true));
