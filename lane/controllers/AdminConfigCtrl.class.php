...
use \Wiki\util\AdminQuickLinks;

class AdminConfigCtrl extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			$this->save();
			$this->form->get_field_by_id('number_character_to_cut')->set_hidden($this->config->get_display_type() === ModConfig::DISPLAY_TABLE);
			$this->form->get_field_by_id('number_cols_display_per_line')->set_hidden($this->config->get_display_type() !== ModConfig::DISPLAY_MOSAIC);
		...
		return new AdminQuickLinks($view, $this->lang['config.title']);
	}

	private function init()
	{
		$this->lang = LangLoader::get('common', 'wiki');
		$this->admin_lang = LangLoader::get('admin-common');
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldCheckbox('display_icon_cats', $this->lang['config.display.categories.icon'], $this->config->are_cats_icon_enabled()));

		$fieldset->add_field(new FormFieldCheckbox('display_color_cats', $this->lang['config.display.categories.color'], $this->config->are_cats_color_enabled()));

		$fieldset->add_field(new FormFieldCheckbox('display_descriptions_to_guests', $this->lang['config.display.desc.to.guests'], $this->config->are_descriptions_displayed_to_guests()));
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['config.display.type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->lang['config.block.view'], ModConfig::DISPLAY_MOSAIC),
				new FormFieldSelectChoiceOption($this->lang['config.list.view'], ModConfig::DISPLAY_LIST),
				new FormFieldSelectChoiceOption($this->lang['config.table.view'], ModConfig::DISPLAY_TABLE)
			),
			array('events' => array('change' => '
				if (HTMLForms.getField("display_type").getValue() === \'' . ModConfig::DISPLAY_MOSAIC . '\') {
					HTMLForms.getField("number_character_to_cut").enable();
					HTMLForms.getField("number_cols_display_per_line").enable();
				} else if (HTMLForms.getField("display_type").getValue() === \'' . ModConfig::DISPLAY_LIST . '\') {
					HTMLForms.getField("number_character_to_cut").enable();
		...
		$fieldset->add_field(new FormFieldNumberEditor('number_character_to_cut', $this->lang['config.characters.number.to.cut'], $this->config->get_number_character_to_cut(),
			array('min' => 20, 'max' => 1000, 'required' => true,'hidden' => $this->config->get_display_type() === ModConfig::DISPLAY_TABLE),
			array(new FormFieldConstraintIntegerRange(20, 1000))
		));

		$fieldset->add_field(new FormFieldNumberEditor('number_cols_display_per_line', $this->admin_lang['config.columns_number_per_line'], $this->config->get_number_cols_display_per_line(),
			array(
				'min' => 1,
				'max' => 6,
				'required' => true,
				'description' => $this->admin_lang['config.columns_number_per_line.description'],
				'hidden' => $this->config->get_display_type() !== ModConfig::DISPLAY_MOSAIC
			),
			array(new FormFieldConstraintIntegerRange(1, 6))
		));

		$fieldset->add_field(new FormFieldRichTextEditor('root_category_description', $this->admin_lang['config.root.category.description'], $this->config->get_root_category_description(),
			array('rows' => 8, 'cols' => 47)
		));

		$common_lang = LangLoader::get('common');
		$fieldset_authorizations = new FormFieldsetHTML('authorizations', $common_lang['authorizations'],
			array('description' => $this->admin_lang['config.authorizations.explain'])
		);
	...
	private function save()
	{
		...
		Config::save();
		ModServices::get_categories_manager()->regenerate_cache();
	}
}
?>
