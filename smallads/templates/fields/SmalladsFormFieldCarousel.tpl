...
SmalladsFormFieldCarousel.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<input/> ', {type : 'text', id : 'field_name_' + id, name : 'field_name_' + id, placeholder : '{@smallads.form.image.description}'}).appendTo('#' + id);
			jQuery('#' + id).append(' ');

			jQuery('<input/> ', {type : 'text', id : 'field_value_' + id, name : 'field_value_' + id, class : 'slider-url', placeholder : '{@smallads.form.image.url}'}).appendTo('#' + id);
			jQuery('#' + id).append(' ');
...
# START fieldelements #
		<div id="${escape(ID)}_{fieldelements.ID}">
			<input type="text" name="field_name_${escape(ID)}_{fieldelements.ID}" id="field_name_${escape(ID)}_{fieldelements.ID}" value="{fieldelements.NAME}" placeholder="{@smallads.form.image.description}"/>
			<input type="text" name="field_value_${escape(ID)}_{fieldelements.ID}" id="field_value_${escape(ID)}_{fieldelements.ID}" value="{fieldelements.VALUE}" placeholder="{@smallads.form.image.url}" class="slider-url"/>
