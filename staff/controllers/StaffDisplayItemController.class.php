...
class StaffDisplayItemController extends ModuleController
{
	...
	{
		$member_name = $this->member->get_firstname() . ' ' . $this->member->get_lastname();

		$email_form = new HTMLForm(__CLASS__);

		$fieldset = new FormFieldsetHTML('send_a_mail', $this->lang['email.member.contact'], array('description' => $member_name));
	...
	private function send_member_email()
	{
		$email_message = '';
		...
		$email_recipient_email = $this->get_member()->get_member_email();
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($member->get_lastname() . ' ' .$member->get_firstname(), $this->lang['staff.module.title']);
