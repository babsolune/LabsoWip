<?php
/*##################################################
 *		                         AdminPalmaresConfigController.class.php
 *                            -------------------
 ...
 */
class AdminPalmaresConfigController extends AdminModuleController
{
	/**
	 * @var HTMLForm
	 */
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		return new AdminPalmaresDisplayResponse($tpl, $this->lang['module_config_title']);
	}
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldNumberEditor('number_palmares_per_page', $this->admin_common_lang['config.items_number_per_page'], $this->config->get_number_palmares_per_page(), 
			array('min' => 1, 'max' => 50, 'required' => true),
			array(new FormFieldConstraintIntegerRange(1, 50))
		));
		
		$fieldset->add_field(new FormFieldNumberEditor('number_columns_display_palmares', $this->admin_common_lang['config.columns_number_per_line'], $this->config->get_number_columns_display_palmares(), 
		...
		$fieldset->add_field(new FormFieldCheckbox('palmares_suggestions_enabled', $this->lang['admin.config.palmares_suggestions_enabled'], $this->config->get_palmares_suggestions_enabled()));
		...
		$fieldset->add_field(new FormFieldCheckbox('nb_view_enabled', $this->lang['admin.config.palmares_number_view_enabled'], $this->config->get_nb_view_enabled()));
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->admin_common_lang['config.display_type'], $this->config->get_display_type(),
			array(
				new FormFieldSelectChoiceOption($this->admin_common_lang['config.display_type.block'], PalmaresConfig::DISPLAY_BLOCK),
				new FormFieldSelectChoiceOption($this->admin_common_lang['config.display_type.list'], PalmaresConfig::DISPLAY_LIST),
			)
		));
	...
	{
		$this->config->set_number_palmares_per_page($this->form->get_value('number_palmares_per_page'));
		$this->config->set_number_columns_display_palmares($this->form->get_value('number_columns_display_palmares'));
		...
		$this->config->set_palmares_suggestions_enabled($this->form->get_value('palmares_suggestions_enabled'));
		...
		PalmaresConfig::save();
		PalmaresService::get_categories_manager()->regenerate_cache();
	}
}
?>