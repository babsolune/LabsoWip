...
class ModcatartItemFormController extends ModuleController
{
	...
	private function build_form(HTTPRequestCustom $request)
	{
		...
		if (ModcatartAuthorizationsService::check_authorizations($this->get_itemcatart()->get_id_category())->moderation())
		{
		...
		if (ModcatartService::get_categories_manager()->get_categories_cache()->has_categories())
		{
			...
			$fieldset->add_field(ModcatartService::get_categories_manager()->get_select_categories_form_field('id_category', $this->common_lang['form.category'], $this->get_itemcatart()->get_id_category(), $search_category_children_options));
		}
	...
	private function get_itemcatart()
	{
		if ($this->itemcatart === null)
		{
			...
			else
			{
				...
				$this->itemcatart->init_default_properties(AppContext::get_request()->get_getint('id_category', Category::ROOT_CATEGORY));
			}
		}
	...
	private function save()
	{
		...
		if (ModcatartService::get_categories_manager()->get_categories_cache()->has_categories())
			$itemcatart->set_id_category($this->form->get_value('id_category')->get_raw_value());
		...
		if (!ModcatartAuthorizationsService::check_authorizations($itemcatart->get_id_category())->moderation())
		{
			if ($itemcatart->get_id() === null)
			...
			if (ModcatartAuthorizationsService::check_authorizations($itemcatart->get_id_category())->contribution() && !ModcatartAuthorizationsService::check_authorizations($itemcatart->get_id_category())->write())
			...
			if ($this->is_contributor_member())
			{
				...
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						ModcatartService::get_categories_manager()->get_heritated_authorizations($itemcatart->get_id_category(), Category::MODERATION_AUTHORIZATIONS, Authorizations::AUTH_CHILD_PRIORITY),
	...
	private function build_response(View $tpl)
	{
		...
		if ($itemcatart->get_id() === null)
		{
			$breadcrumb->add($this->lang['modcatart.add'], ModcatartUrlBuilder::add_item($itemcatart->get_id_category()));
			...
			$graphical_environment->get_seo_meta_data()->set_canonical_url(ModcatartUrlBuilder::add_item($itemcatart->get_id_category()));
		}
		else
		{
			$categories = array_reverse(ModcatartService::get_categories_manager()->get_parents($itemcatart->get_id_category(), true));
