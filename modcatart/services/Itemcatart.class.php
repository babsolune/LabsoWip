...
class Itemcatart
{
	...
	{
		return ModcatartService::get_categories_manager()->get_categories_cache()->get_category($this->id_category);
	}
	...
	{
		$now = new Date();
		return ModcatartAuthorizationsService::check_authorizations($this->id_category)->read() && ($this->get_publication_state() == self::PUBLISHED_NOW || ($this->get_publication_state() == self::PUBLICATION_DATE && $this->get_publication_start_date()->is_anterior_to($now) && ($this->enabled_end_date ? $this->get_publication_end_date()->is_posterior_to($now) : true)));
	}
	...
	{
		return ModcatartAuthorizationsService::check_authorizations($this->id_category)->write() || ModcatartAuthorizationsService::check_authorizations($this->id_category)->contribution();
	}

	public function is_authorized_to_edit()
	{
		return ModcatartAuthorizationsService::check_authorizations($this->id_category)->moderation() || ((ModcatartAuthorizationsService::check_authorizations($this->get_id_category())->write() || (ModcatartAuthorizationsService::check_authorizations($this->get_id_category())->contribution() && !$this->is_published())) && $this->get_author_user()->get_id() == AppContext::get_current_user()->get_id() && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL));
	}
