...
class ClubsSearchable extends AbstractSearchableExtensionPoint
{
	public function get_search_request($args)
	{
		...
		return "SELECT " . $args['id_search'] . " AS id_search,
			...
			CONCAT('" . PATH_TO_ROOT . "/clubs/" . (!ServerEnvironmentConfig::load()->is_url_rewriting_enabled() ? "index.php?url=/" : "") . "', id_category, '-', IF(id_category != 0, cat.rewrited_name, 'root'), '/', w.id, '-', w.rewrited_name) AS link
			FROM " . ClubsSetup::$clubs_table . " w
			LEFT JOIN ". ClubsSetup::$clubs_cats_table ." cat ON w.id_category = cat.id
