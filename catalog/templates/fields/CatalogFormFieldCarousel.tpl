...
CatalogFormFieldCarousel.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:CatalogFormFieldCarousel.delete_field('+ this.integer +');'}).html('<i class="fa fa-delete"></i>').appendTo('#' + id);
...
var CatalogFormFieldCarousel = new CatalogFormFieldCarousel();
-->
</script>
...
# START fieldelements #
		<div id="${escape(ID)}_{fieldelements.ID}">
			...
			<a href="javascript:CatalogFormFieldCarousel.delete_field({fieldelements.ID});" data-confirmation="delete-element"><i class="fa fa-delete"></i></a>
		</div>
# END fieldelements #
</div>
<a href="javascript:CatalogFormFieldCarousel.add_field();" id="add-${escape(ID)}"><i class="fa fa-plus"></i></a>
