<?php
/*##################################################
 *                               CatalogAuthorizationsService.class.php
 *                            -------------------
...
class CatalogAuthorizationsService
{
	public $id_category;
	
	const DISPLAY_PRODUCT_AUTHORIZATIONS = 32;
	...
	{
		return $this->is_authorized(self::DISPLAY_PRODUCT_AUTHORIZATIONS);
	}
	...
	{
		if ($bit == self::DISPLAY_PRODUCT_AUTHORIZATIONS)
			$auth = CatalogConfig::load()->get_authorizations();
		else
			$auth = CatalogService::get_categories_manager()->get_heritated_authorizations($this->id_category, $bit, $mode);
