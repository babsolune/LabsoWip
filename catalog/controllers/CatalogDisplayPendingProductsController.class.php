<?php
/*##################################################
 *                               CatalogDisplayPendingProductsController.class.php
 *                            -------------------
...
class CatalogDisplayPendingProductsController extends ModuleController
{
	private $tpl;
	private $lang;
	...
	public function build_view(HTTPRequestCustom $request)
	{
		...
		' . (!CatalogAuthorizationsService::check_authorizations()->moderation() ? ' AND author_user_id = :user_id' : '') . '
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, 
			...
			array('events' => array('change' => 'document.location = "'. CatalogUrlBuilder::display_pending()->rel() . '" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();'))
		));
	...
	private function get_pagination($condition, $parameters, $field, $mode, $page)
	{
		...
		$pagination->set_url(CatalogUrlBuilder::display_pending($field, $mode, '%d'));
	...
	{
		if (!(CatalogAuthorizationsService::check_authorizations()->write() || CatalogAuthorizationsService::check_authorizations()->contribution() || CatalogAuthorizationsService::check_authorizations()->moderation()))
		{
	...
	private function generate_response()
	{
		...
		$graphical_environment->set_page_title($this->lang['catalog.pending'], $this->lang['module_title']);
		$graphical_environment->get_seo_meta_data()->set_description($this->lang['catalog.seo.description.pending']);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(CatalogUrlBuilder::display_pending(AppContext::get_request()->get_getstring('field', 'date'), AppContext::get_request()->get_getstring('sort', 'desc'), AppContext::get_request()->get_getint('page', 1)));
		
		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['module_title'], CatalogUrlBuilder::home());
		$breadcrumb->add($this->lang['catalog.pending'], CatalogUrlBuilder::display_pending());
