<?php
/*##################################################
 *                          TsmClubsDeadLinkController.class.php
 *                            -------------------
...
class TsmClubsDeadLinkController extends AbstractController
{
	private $club;
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		if ($this->club !== null && $this->club->is_published())
		{
			if (!PersistenceContext::get_querier()->row_exists(PREFIX . 'events', 'WHERE id_in_module=:id_in_module AND module=\'clubs\' AND current_status = 0', array('id_in_module' => $this->club->get_id())))
			{
				...
				$contribution->set_fixing_url(TsmUrlBuilder::edit_club($this->club->get_id())->relative());
				$contribution->set_description(LangLoader::get_message('contribution.deadlink_explain', 'common'));
				$contribution->set_poster_id(AppContext::get_current_user()->get_id());
				$contribution->set_module('tsm');
				$contribution->set_type('alert');
				$contribution->set_auth(
					Authorizations::capture_and_shift_bit_auth(
						TsmConfig::load()->get_authorizations($this->club->get_id(), self::MODERATION_AUTH),
						self::MODERATION_AUTHORIZATIONS, Contribution::CONTRIBUTION_AUTH_BIT
					)
				);
