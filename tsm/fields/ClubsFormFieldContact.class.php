...
class ClubsFormFieldContact extends AbstractFormField
{
	...
	function display()
	{
		...
		$tpl = new FileTemplate('tsm/fields/ClubsFormFieldContact.tpl');
		$tpl->add_lang(LangLoader::get('club', 'tsm'));
	...
	public function retrieve_value()
	{
		...
		for ($i = 0; $i < $this->max_input; $i++)
		{
			...
			$field_contact_email_id  = 'field_contact_email_' . $this->get_html_id() . '_' . $i;
			...
			if ($request->has_postparameter($field_contact_name_id) || $request->has_postparameter($field_contact_email_id) || $request->has_postparameter($field_contact_phone1_id) || $request->has_postparameter($field_contact_phone2_id) || $request->has_postparameter($field_contact_site_id))
			{
				$field_contact_name		= $request->get_poststring($field_contact_name_id);
				$field_contact_email 	= $request->get_poststring($field_contact_email_id);
